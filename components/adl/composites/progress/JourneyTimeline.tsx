      ...
      {nodes.map((node, i) => (
        <div key={node.id} className={cn(
          ...
          <div className="relative z-10 shrink-0">
            <div className={cn(
              "w-12 h-12 rounded-full border-2 flex items-center justify-center bg-base transition-colors",
              node.isActive 
                ? "border-[var(--color-accent-gold)] text-[var(--color-accent-gold)] shadow-[0_0_15px_rgba(var(--color-accent-gold-rgb),0.3)]"
                : "border-border-subtle text-[var(--color-text-muted)] group-hover:border-[var(--color-text-secondary)]"
            )}>
              {node.icon ? (
