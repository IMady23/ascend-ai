      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-md bg-[var(--color-bg-base)] border border-[var(--color-glass-border)] rounded-[var(--radius-2xl)] shadow-[var(--shadow-2xl)] overflow-hidden flex flex-col"
            role="dialog"
            aria-modal="true"
          >
            {/* Header */}
            <div className="relative p-6 border-b border-[var(--color-glass-border)] bg-[var(--color-bg-surface)]">
              <button
                onClick={onClose}
                className="absolute right-4 top-4 p-2 rounded-full text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] hover:bg-[var(--color-bg-glass-hover)] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent-blue)]"
                aria-label="Close"
              >
                <X size={20} />
              </button>
            ...
            <div className="p-6 space-y-6">
              <BodyText size="md" className="text-[var(--color-text-secondary)]">
                {description}
              </BodyText>
            ...
            <div className="p-4 bg-[var(--color-bg-surface)] border-t border-[var(--color-glass-border)] flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-[var(--color-text-secondary)]">Under Development</span>
              </div>
