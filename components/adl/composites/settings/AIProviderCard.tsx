...
export function AIProviderCard({
  ...
  return (
    <GlassCard className={cn("p-5 border flex flex-col gap-4", isActive ? "border-[var(--current-accent,var(--color-accent-indigo))] bg-[var(--current-accent,var(--color-accent-indigo))]/5 shadow-sm" : "border-[var(--color-glass-border)] opacity-70 hover:opacity-100", className)}>
      
      <div className="flex justify-between items-start">
        <div className="flex items-center gap-3">
          <div className={cn("p-2 rounded-lg", isActive ? "bg-[var(--current-accent,var(--color-accent-indigo))]/20 text-[var(--current-accent,var(--color-accent-indigo))]" : "bg-[var(--color-bg-base)] text-[var(--color-text-muted)]")}>
            <BrainCircuit size={20} />
          </div>
          <div>
            <Heading level="h4" className="text-base">{providerName}</Heading>
            <Caption className="text-[var(--color-text-secondary)] font-mono">{modelName}</Caption>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-2 pt-3 border-t border-[var(--color-glass-border)]">
        <div className="flex flex-col items-center justify-center p-2 rounded-md bg-[var(--color-bg-surface)]">
