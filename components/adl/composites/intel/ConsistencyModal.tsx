      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed inset-x-4 bottom-4 md:inset-auto md:top-1/2 md:left-1/2 md:-translate-x-1/2 md:-translate-y-1/2 z-50 md:w-full max-w-md bg-[var(--color-bg-base)] border border-[var(--color-glass-border)] rounded-[var(--radius-2xl)] shadow-2xl flex flex-col overflow-hidden"
          >
            {/* Header */}
            <div className="flex items-center justify-between p-6 border-b border-[var(--color-glass-border)] bg-gradient-to-br from-[var(--color-accent-orange)]/10 to-transparent">
              <div>
                <Heading level="h2" className="text-xl">Consistency Score</Heading>
                <Caption className="text-[var(--color-text-secondary)]">Your adherence breakdown.</Caption>
              </div>
            ...
            <div className="flex-1 p-6 overflow-y-auto space-y-6">
              ...
              {/* Score Headline */}
              <div className="flex items-center justify-between bg-[var(--color-bg-surface)] p-4 rounded-xl border border-[var(--color-glass-border)]">
                <div>
                  <Caption className="text-[var(--color-text-muted)] uppercase tracking-widest font-semibold mb-1">Current Score</Caption>
                  <div className="flex items-baseline gap-2">
                    <span className="text-4xl font-bold text-white">{consistencyBreakdown.currentScore}%</span>
                    ...
                    {consistencyBreakdown.trend30Day === "stable" && <Minus size={20} className="text-[var(--color-text-secondary)]" />}
                  </div>
                </div>
                <div className="text-right">
                  <Caption className="text-[var(--color-text-muted)] uppercase tracking-widest font-semibold mb-1">Previous Score</Caption>
                  <span className="text-2xl font-bold text-[var(--color-text-secondary)]">{consistencyBreakdown.previousScore}%</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4">
                <div className="p-4 rounded-xl border border-[var(--color-success)]/30 bg-[var(--color-success)]/5">
                  ...
                  <BodyText size="sm" className="text-white">{consistencyBreakdown.biggestPositive}</BodyText>
                </div>
                <div className="p-4 rounded-xl border border-[var(--color-error)]/30 bg-[var(--color-error)]/5">
                  ...
                  <BodyText size="sm" className="text-white">{consistencyBreakdown.biggestNegative}</BodyText>
                </div>
              </div>

              {/* How to improve */}
              <div className="p-4 rounded-xl border border-[var(--color-glass-border)] bg-[var(--color-bg-surface)] flex gap-3">
                <Info className="text-[var(--color-accent-blue)] shrink-0 mt-0.5" size={18} />
                <div>
                  <Caption className="text-[var(--color-text-secondary)] font-bold mb-1">How to Improve</Caption>
                  <BodyText size="sm" className="text-[var(--color-text-secondary)]">{consistencyBreakdown.howToImprove}</BodyText>
                </div>
              </div>
  ...
  return (
    <div className="flex items-center justify-between">
      ...
      <div className="flex items-center gap-3 w-1/2">
        <div className="flex-1 h-2 rounded-full bg-[var(--color-bg-surface)] overflow-hidden">
