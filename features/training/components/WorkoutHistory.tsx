      ...
      <div className="divide-y divide-zinc-800/50">
        {historyList.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-12 text-center">
            ...
            <p className="text-secondary font-medium">No workouts logged yet.</p>
            <p className="text-disabled text-sm mt-2">Your completed sessions will appear here.</p>
          </div>
        ) : (
          historyList.map((item, i) => (
            <div key={item.id || i} className="p-6 hover:bg-surface-elevated/20 transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-base border border-border-subtle rounded-xl flex items-center justify-center text-secondary">
                  <Dumbbell size={20} />
                </div>
                <div>
                  <h3 className="font-bold text-primary text-lg">{item.type}</h3>
                  <p className="text-secondary text-sm">{formatDate(item.date)}</p>
                </div>
              </div>
